...
async def list_watched_lecture_cards(enrollment_number: str = Query(..., min_length=3, max_length=32)) -> ResponseBase:
    ...
    # Transform "lectures" key to "cards" for the response
    cards_payload = {"cards": payload.get("lectures", [])}
    return ResponseBase(status=True, message="Watched lectures fetched successfully", data=cards_payload)
